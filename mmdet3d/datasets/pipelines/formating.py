    ...
    def __call__(self, results):
        ...
        if 'img_depth' in results:
            if isinstance(results['img_depth'], list):
                # process multiple imgs in single frame
                img_depths = np.ascontiguousarray(np.stack(results['img_depth'], axis=0))
                results['img_depth'] = DC(to_tensor(img_depths), stack=True)
            else:
                img_depths = np.ascontiguousarray(results['img_depth'])
                results['img_depth'] = DC(to_tensor(img_depths), stack=True)
...
class Collect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            'lidar2cam', 'cam2lidar', 'depth2img', 'cam2img', 'pad_shape',
