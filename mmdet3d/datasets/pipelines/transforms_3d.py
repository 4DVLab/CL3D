...
from mmdet.datasets.pipelines import RandomFlip, Pad
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if isinstance(results[key], list):
                ...
                results[key] = norm_imgs
            else:
                assert 0, 'NormalizeMultiview only support a list of images!'
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        return results

@PIPELINES.register_module()
class PadMultiview(Pad):
    ...
    def __init__(self,
        ...
        super(PadMultiview, self).__init__(
            size=size, size_divisor=size_divisor, 
            pad_to_square=pad_to_square, pad_val=pad_val)
                ...
                for img in results[key]:
                    ...
                    if self.size is not None:
                        padded_img = mmcv.impad(
                            img, shape=self.size, pad_val=pad_val)
                    elif self.size_divisor is not None:
                        padded_img = mmcv.impad_to_multiple(
                            img, self.size_divisor, pad_val=pad_val)
                    padded_imgs.append(padded_img) 
                results[key] = padded_imgs
            else:
                assert 0, 'PadMultiview only support a list of images!'
        results['pad_shape'] = (*padded_img.shape[:2], *results['pad_shape'][-2:])
