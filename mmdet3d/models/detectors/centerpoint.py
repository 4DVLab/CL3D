...
class CenterPoint(MVXTwoStageDetector):
    ...
    def set_DA_mode(self, status):
        self.pts_bbox_head.DA_mode = status
    
    def get_DA_mode(self):
        return self.pts_bbox_head.DA_mode
    
    def set_prototype_mode(self, status):
        self.pts_bbox_head.prototype_mode = status
    
    def get_prototype_mode(self):
        return self.pts_bbox_head.prototype_mode

    def init_prototypes(self):
        prototypes = self.pts_bbox_head.prototypes

        for i, prototype in enumerate(prototypes):
            prototype = torch.stack(prototype, dim=0)
            prototype = torch.mean(prototype, dim=0)
            self.pts_bbox_head.prototypes[i] = prototype
    ...
    def forward_pts_train(self,
        ...
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs, pts_feats[0], points]
