...
def parse_args():
    ...
    parser.add_argument(
        '--prefix',
        type=str,
        help='the prefix of the generated pseudo label')
    parser.add_argument(
        '--eval',
        type=str,
        nargs='+',
...
def main():
    ...
    print("Load data from {}...".format(cfg.data.test.ann_file))
    ...
    if rank == 0:
        ...
        result_files, _ = dataset.format_results(outputs, **kwargs)
        ...
        for idx, key in enumerate(preds):
            ...
            # info = {
            #     'lidar_path': lidar_path,
            #     'gt_boxes': np.array(pred_boxes, dtype=np.float32),
            #     'gt_names': np.array(pred_names),
            # }
            infos.append(info)
        if args.prefix is not None:
            info_path = os.path.join(save_dir, '{}_pseudo_'.format(args.prefix) + save_name)
        else:
            info_path = os.path.join(save_dir, 'pseudo_' + save_name)
        
        with open(info_path, 'wb') as f:
            pickle.dump(infos, f)
        print('Pseudo label save at {}...'.format(info_path))
